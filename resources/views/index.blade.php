    ...
    /*
        ...
        modal.setContent(`
                  @foreach($skuList as $sku)
                    <input type='radio' class='form-check-input' name='selectedSKU' value='{{ $sku->id }}'>{{ $sku->sku_name }} <br/>
                  @endforeach
        ...
        domComps.addType('sku', {
            model: dModel.extend({
              init() {
               ...
               doStuff() {
                 this.render;
                 var skuId = this.get('traits').where({name: 'sku'})[0].get('value');
                ...
                traits: [
                  {
                    ...
                    changeProp: 1,
                    options: [
                      {value: '1', name: 'Grace Promises For Moms (Hardback)'},
                      {value: '2', name: 'The Prayer of Protection–Living Fearlessly in Dangerous Times'},
                    ]
                  }],
              }),
            }, {
...
//ACTION LISTENER
/*editor.on('canvas:dragend', function(){
  console.log(this);
})*/
...
window.editor = editor;
        ...
        <!-- POP UP MODAL -->
        <div class='modal fade' id='exampleModal' data-target='#exampleModal'  role='dialog' aria-labelledby='exampleModalLabel' aria-hidden='true'>
                ...
                <div class='modal-content'>
                    ...
                    <div class='modal-body'>
                      <div class='form-check'>i
                        @foreach($skuList as $sku)
                          <input type='radio' class='form-check-input' name='selectedSKU' value='{{ $sku->id }}'>{{ $sku->sku_name }}
                        @endforeach
                      </div>
                    </div>
                    <div class='modal-footer'>
